...
* Elements should have properties: x, y, width, height, z-index, color/text\_color, and content.
...
* Low-Level (Computer Use): mouse\_move(x, y), mouse\_click(), mouse\_drag(x1, y1, x2, y2), keyboard\_type(string).  
* High-Level (Semantic UI): add\_element(type, content), move\_element(id, new\_x, new\_y), change\_element\_color(id, hex\_code).
...
Wrap your environment in an [MCP Server](https://modelcontextprotocol.io/).

* Expose tools like get\_canvas\_state, execute\_action, and get\_current\_reward.  
...
1. Python Source Code: Clean, well-documented code for the environment (feel free to use gymnasium structures) and the MCP server.  
2. Demo Script (demo.py): A script that initializes the environment, accepts a mock target prompt, takes a few programmatic (or random) steps, and prints the resulting state and reward.  
3. WRITEUP.md: A 1-2 page document covering:  
...
4. (Optional) Visual Output: A simple mechanism to save the final canvas state as a .png or draw it using a simple UI library (e.g., Pygame, Tkinter, or just PIL).
